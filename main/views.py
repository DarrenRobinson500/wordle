...
def find_max(letters):
    max = 0
    letter = ""
    for x in letters:
        if letters.count(x) > max:
            max = letters.count(x)
            letter = x
    return letter
...
def determine_colour(word,entry,position):
    result = "Grey"
    for x in range(0, 5):
        if entry[position-1] == word[x]: result = 'Orange'
    if entry[position-1] == word[position-1]: result = "Green"
    return result
...
def most_popular_letter(letters):
    unique = set(letters)
    max = 0
    max_letter = ''
    for x in unique:
...
def test_logic(word):
    ...
    # Round 2
    entry = fav_word1
    input = [(entry[0], determine_colour(word,entry,1)), (entry[1], determine_colour(word,entry,2)), (entry[2], determine_colour(word,entry,3)), (entry[3], determine_colour(word,entry,4)), (entry[4], determine_colour(word,entry,5)), ]
    input_array.append(input)
...
def home(request, entry=None):
    ...
    context = {'words': words, 'numbers': numbers, 'fav_word': fav_word, 'count': len(words), 'input_array': input_array,
...
def check_old(position, colour, value, word):
    if colour == "Green" and word[position] != value: return False
    if colour == "Grey":
        for x in range(0,5):
            if word[x] == value: return False
    if colour == "Orange":
        found = False
        for x in range(0,5):
            if word[x] == value: found = True
...
def home_old(request, entry=None):
    ...
    if request.method == 'POST':
        # for x in request.POST:
        value1 = request.POST.__getitem__('1')
        value2 = request.POST.__getitem__('2')
        value3 = request.POST.__getitem__('3')
        value4 = request.POST.__getitem__('4')
        value5 = request.POST.__getitem__('5')
        try: colour1 = request.POST.__getitem__('1x')
        except: colour1 = ""
        try: colour2 = request.POST.__getitem__('2x')
        except: colour2 = ""
        try: colour3 = request.POST.__getitem__('3x')
        except: colour3 = ""
        try: colour4 = request.POST.__getitem__('4x')
        except: colour4 = ""
        try: colour5 = request.POST.__getitem__('5x')
        except: colour5 = ""

    input = [(value1, colour1), (value2, colour2), (value3, colour3), (value4, colour4), (value5, colour5), ]
    valid = True
    for x, y in input:
        if x == "" or y == "": valid = False
    if valid:
    ...
    if len(input_array) > 0:
        for word in all_words:
            ...
            if valid:
                record = Word.objects.filter(word=word).first()
                words.append(str(record))
                letters1.append(word[0])
                letters2.append(word[1])
                letters3.append(word[2])
                letters4.append(word[3])
                letters5.append(word[4])
